...
const NotesView = () => {
  ...
  const fetchNotes = async () => {
    ...
    try {
      ...
      const transformed = resp.data.map(n => ({
        id: n.id,
        title: n.title,
        subject: n.course_name || 'General',
        chapter: 'Resource',
        date: new Date().toLocaleDateString(), // Backend doesn't have date in model yet
        type: n.file_type, // pdf, video, link
        color: n.file_type === 'pdf' ? 'bg-blue-100 text-blue-700' : 
               n.file_type === 'video' ? 'bg-orange-100 text-orange-700' : 
               'bg-emerald-100 text-emerald-700',
        tag: n.file_type.toUpperCase(),
        content: n.url,
        url: n.url
      }));
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
        {loading ? (
             <div className="col-span-full py-20 text-center text-slate-400">Loading resources...</div>
        ) : filtered.length === 0 ? (
             <div className="col-span-full py-20 text-center text-slate-400">No resources found matching your criteria.</div>
        ) : (
          filtered.map((note, i) => {
