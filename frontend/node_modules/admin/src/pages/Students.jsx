...
import { FiUserPlus, FiFilter, FiEdit2, FiTrash2, FiCheckSquare, FiCheckCircle, FiEye, FiEyeOff } from 'react-icons/fi';
    ...
    const fetchStudents = async () => {
        ...
        try {
            ...
            const fetched = res.data.map(stu => ({
                ...
                email: stu.email || 'N/A',
                rawPassword: stu.rawPassword || 'N/A',
                fullData: stu
            }));
            setStudents(fetched);
        } catch (err) {
    ...
    const handleSaveStudent = async (formData, editId) => {
        ...
        try {
            if (editId) {
                ...
                const newTableEntry = {
                    id: 'STU' + (res.data.id || res.data._id).toString().substring((res.data.id || res.data._id).toString().length - 4).toUpperCase(),
                    name: res.data.fullName,
                    email: res.data.email,
                    rawPassword: res.data.rawPassword,
                    class_name: res.data.className || res.data.class_name || 'N/A',
                            ...
                            <tr key={student.id} style={{ backgroundColor: selectedStudentIds.includes(student.id) ? 'rgba(37, 99, 235, 0.05)' : 'transparent' }}>
                                ...
                                <td>
                                    <div className="flex items-center gap-2" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <span>
                                            {visiblePasswords[student.id] ? '••••••••' : student.rawPassword}
                                        </span>
                                        <button 
                                            ...
                                            title={visiblePasswords[student.id] ? "Show Password" : "Hide Password"}
                                        >
                                            {visiblePasswords[student.id] ? <FiEye size={16} /> : <FiEyeOff size={16} />}
                                        </button>
                                    </div>
                                </td>
